...
class runAlg{
	public static void main(String args[]){
		...
		// 処理クラスを設定
		clsProcess[] cProcess = new clsProcess[2];
		cProcess[0] = new clsProcess0();  // アルゴリズム用乱数生成
		cProcess[1] = new clsProcess1();  // 線形探索法（リニアサーチ）
		
		// パラメータチェック
		boolean bPara =chkPara(args);
		if (bPara == false){
			for (int i = 0; i <= clsConst.MAX_PROCESS; i++){
				out.println(i + "：" + cProcess[i].sPro_Name);
			}
			return;
		}
		...
		if (iNum <= clsConst.MAX_PROCESS){
			cProcess[iNum].args = args;
			cProcess[iNum].subMain();
		} else {
...
abstract class clsProcess implements itfProcess{
	...
	// 処理毎変数初期セット
	int iParaCnt;
	String sParaGuide = "ガイド未設定";
	String sPro_Name = "処理：未実装";
	String sPro_Desc = "処理概要：未実装";
	...
	public void subMain(){
		// タイトル表示
		out.println(sPro_Name + clsConst.NEW_LINE + sPro_Desc + clsConst.NEW_LINE);
	...
	protected boolean chkPara(){
		...
		if (args.length > 1){
			if (args[1].equals("?")){
				// パラメータガイドメッセージ表示
				out.println(sParaGuide);
				bPara = false;
				return bPara;
			}
		}
		if (args.length != iParaCnt){
...
class clsProcess0 extends clsProcess{
	...
	clsProcess0(){
		// 処理毎変数セット
		iParaCnt = 2;
		sParaGuide = "パラメータ１：処理番号（0固定）" + clsConst.NEW_LINE;
		sParaGuide += "パラメータ２：数値（乱数上限）" + clsConst.NEW_LINE;
		sPro_Name = "アルゴリズム用乱数生成（乱数を表示させるだけ）";
		sPro_Desc = "アルゴリズム処理用の乱数を作成する処理（乱数を表示させるだけ）";
	}
	// 個別チェック処理
...
class clsProcess1 extends clsProcess{
	...
	clsProcess1(){
		...
		sParaGuide = "パラメータ１：処理番号（0固定）" + clsConst.NEW_LINE;
		sParaGuide += "パラメータ２：数値（乱数上限）" + clsConst.NEW_LINE;
		sParaGuide += "パラメータ３：数値（検索数値）" + clsConst.NEW_LINE;
		sPro_Name = "未実装：線形探索法（リニアサーチ）";
		sPro_Desc = "未実装：線形探索法のプログラム演習";
	}
	// 個別チェック処理
	protected boolean chkParaOriginal(){
		boolean bPara = true;
